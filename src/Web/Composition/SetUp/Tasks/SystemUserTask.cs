...
namespace Web.Composition.SetUp.Tasks
{
	public class SystemUserTask : ISetUpTask
	{
		...
		public async Task ExecuteAsync(CancellationToken cancellationToken)
		{
			try
			{
				var systemRole = await _roleManager.FindByNameAsync(NamedRoles.System);
				...
				if (!hasSystemUser)
				{
					...
					if (result.IsSuccess)
					{
						...
						var roleResult = await _userManager.AddToRoleAsync(systemUser!, NamedRoles.System);
						...
						{
							throw new InvalidOperationException($"System user could not be assigned the role '{NamedRoles.System}'");
						}
					}
					else
					{
